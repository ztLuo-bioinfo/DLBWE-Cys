...
for i in range(len(class_list)):
    ...
    accuracy = (np.array(file.iloc[:10, 1])/100).tolist()
    index_list += accuracy
...
# 优化y轴的范围以适应显著性标记
plt.ylim(0.62, y + len(compaired) * h + 0.02)  # 留出足够的空间来展示显著性标记
plt.title('')
plt.xlabel('')
...
plt.savefig('./ind_ACC_dif.jpg',dpi=2000)
plt.savefig('./ind_ACC_dif.pdf',dpi=2000)
