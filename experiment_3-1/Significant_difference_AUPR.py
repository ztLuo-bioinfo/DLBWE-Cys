...
for i in range(len(class_list)):
    ...
    accuracy = (np.array(file.iloc[:10, 6])).tolist()
    index_list += accuracy
...
# 优化y轴的范围以适应显著性标记
plt.ylim(0.66, y + len(compaired) * h + 0.02)  # 留出足够的空间来展示显著性标记
plt.title('')
plt.xlabel('')
...
# plt.savefig('./ind_AUPR_dif.jpg',dpi=2000)
# plt.savefig('./ind_AUPR_dif.pdf',dpi=2000)
