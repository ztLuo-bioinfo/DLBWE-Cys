...
# 加载特征和标签数据
att = load_npy('./att.npy')
bilstm = load_npy('./BiLSTM.npy')
cnn = load_npy('./CNN.npy')
fnn = load_npy('./FNN.npy')
input_feature = load_npy('./input_feature.npy')
labels = load_npy('./label.npy')

# 整合并重塑特征数据，确保第一维为1212
att_consolidated = consolidate_features(att).reshape(304, -1)
bilstm_consolidated = consolidate_features(bilstm).reshape(304, -1)
cnn_consolidated = consolidate_features(cnn).reshape(304, -1)
fnn_consolidated = consolidate_features(fnn).reshape(304, -1)
input_feature_consolidated = consolidate_features(input_feature).reshape(304, -1)
...
combined_fnn_tsne = tsne_att.fit_transform(fnn_consolidated)
...
def plot_tsne(tsne_result, labels, title):
    ...
    # plt.savefig('./TSNE_ind_Attention.jpg', dpi=2000)
    # plt.savefig('./TSNE_ind_Attention.pdf', dpi=2000)
