...
# 加载特征和标签数据
att = load_npy('./att_train.npy')
bilstm = load_npy('./BiLSTM_train.npy')
cnn = load_npy('./CNN_train.npy')
fnn = load_npy('./FNN_train.npy')
input_feature = load_npy('./input_feature_train.npy')
labels = load_npy('./label_train.npy')

# 整合并重塑特征数据，确保第一维为1212
att_consolidated = consolidate_features(att).reshape(1204, -1)
bilstm_consolidated = consolidate_features(bilstm).reshape(1204, -1)
cnn_consolidated = consolidate_features(cnn).reshape(1204, -1)
fnn_consolidated = consolidate_features(fnn).reshape(1204, -1)
input_feature_consolidated = consolidate_features(input_feature).reshape(1204, -1)
...
def plot_tsne(tsne_result, labels, title):
    ...
    # plt.savefig('./TSNE_feature_train.jpg', dpi=2000)
    # plt.savefig('./TSNE_feature_train.pdf', dpi=2000)
    # plt.savefig('./TSNE_cnn_train.jpg', dpi=2000)
    # plt.savefig('./TSNE_cnn_train.pdf', dpi=2000)
    # plt.savefig('./TSNE_bilstm_train.jpg', dpi=2000)
    # plt.savefig('./TSNE_bilstm_train.pdf', dpi=2000)
    # plt.savefig('./TSNE_att_train.jpg', dpi=2000)
    # plt.savefig('./TSNE_att_train.pdf', dpi=2000)
