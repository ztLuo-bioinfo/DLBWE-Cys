...
darker_colors = darken_color('orange', darkening_factor)
...
# 从DataFrame中提取MCC值
mcc_values = (df['ACC']/100).values.flatten().tolist()
mcc_values += mcc_values[:1]  # 添加第一个MCC值在数组的末尾以闭合雷达图

# 设置雷达图的值范围和起始角度
radar_min, radar_max = 0.5, 0.8  # AAC
...
# 微调标题位置以避免与最外环的值重叠
ax.set_title('ACC', size=15, color='k', position=(0.5, 1.15), horizontalalignment='center')

plt.savefig('./radar_train_ACC.jpg',dpi=2000)
plt.savefig('./radar_train_ACC.pdf',dpi=2000)
