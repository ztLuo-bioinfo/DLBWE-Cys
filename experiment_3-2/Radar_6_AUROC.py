...
darker_colors = darken_color('cyan', darkening_factor)
...
# 从DataFrame中提取MCC值
mcc_values = df['AUROC'].values.flatten().tolist()
...
# 微调标题位置以避免与最外环的值重叠
ax.set_title('AUROC', size=15, color='k', position=(0.5, 1.15), horizontalalignment='center')

plt.savefig('./radar_train_AUROC.jpg',dpi=2000)
plt.savefig('./radar_train_AUROC.pdf',dpi=2000)
